...
export async function getProjectStats() {
  try {
    ...
    const vehicleStats = {
      ...
      completed: vehicles?.filter(v => v.status === 'Completed').length || 0,
      inProgress: vehicles?.filter(v => v.status === 'In Progress').length || 0,
      pending: vehicles?.filter(v => v.status === 'Pending').length || 0,
      totalGpsDevices: vehicles?.reduce((sum, v) => sum + v.gps_required, 0) || 0,
      totalFuelSensors: vehicles?.reduce((sum, v) => sum + v.fuel_sensors, 0) || 0
    }
