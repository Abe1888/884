...
const TestingPanel: React.FC = memo(() => {
  ...
  const runDataIntegrityTests = async (): Promise<TestResult[]> => {
    ...
    try {
      ...
      const missingLocations = [...vehicleLocations].filter(loc => !definedLocations.has(loc));
    ...
    try {
      ...
      const taskVehicleIds = new Set(tasks?.map(t => t.vehicle_id) || []);
      const vehicleIds = new Set(vehicles?.map(v => v.id) || []);
      const invalidRefs = [...taskVehicleIds].filter(id => !vehicleIds.has(id));
