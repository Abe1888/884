...
const DataValidationPanel: React.FC = memo(() => {
  ...
  const validationResults = useMemo(() => {
    ...
    if (locations.length === 0) {
      ...
      const undefinedLocations = [...vehicleLocations].filter(loc => !definedLocations.has(loc));
    ...
    if (tasks.length === 0) {
      ...
      const invalidAssignments = tasks.filter(t => !teamMemberNames.has(t.assigned_to));
      if (invalidAssignments.length > 0) {
        results.push({
          category: 'Task Data',
          type: 'warning',
          message: `${invalidAssignments.length} tasks assigned to unknown team members`,
          items: [...new Set(invalidAssignments.map(t => t.assigned_to))]
        });
      }

      // Check for tasks with invalid vehicle references
      const vehicleIds = new Set(vehicles.map(v => v.id));
      const invalidVehicleRefs = tasks.filter(t => !vehicleIds.has(t.vehicle_id));
      if (invalidVehicleRefs.length > 0) {
        results.push({
          category: 'Task Data',
          type: 'error',
          message: `${invalidVehicleRefs.length} tasks reference non-existent vehicles`,
          items: [...new Set(invalidVehicleRefs.map(t => t.vehicle_id))]
        });
      }
