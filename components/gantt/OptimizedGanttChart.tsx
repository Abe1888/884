...
const OptimizedGanttChart = memo(() => {
  ...
  const timelineData = useMemo(() => {
    ...
    const result = { vehicles: filtered.sort((a, b) => a.day - b.day), days, minDay, maxDay };
    renderCache.current.set(cacheKey, result);
    return result;
  }, [vehicles, selectedLocation]);

  const getLocationColor = useCallback((location: string) => {
    const colors = { 'Bahir Dar': 'bg-blue-500', 'Kombolcha': 'bg-green-500', 'Addis Ababa': 'bg-purple-500' };
  ...
  React.useEffect(() => { renderCache.current.clear(); }, [vehicles]);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-white border border-slate-200 rounded-lg">
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            <select value={selectedLocation} onChange={(e) => setSelectedLocation(e.target.value)} className="px-3 py-2 text-sm border border-slate-300 rounded-md">
            ...
            <div className="flex border border-slate-300 rounded-md overflow-hidden">
              <button onClick={() => setViewMode('day')} className={`px-3 py-2 text-sm ${viewMode === 'day' ? 'bg-blue-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-50'}`}>Day View</button>
              <button onClick={() => setViewMode('week')} className={`px-3 py-2 text-sm border-l border-slate-300 ${viewMode === 'week' ? 'bg-blue-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-50'}`}>Week View</button>
            </div>
          </div>
        </div>
      </div>
                  ...
                  <div key={day} className="text-center">
                    <div className="text-xs font-medium text-slate-700">
                      ...
                      <div className="text-xs text-slate-500 mt-1">
                        {new Date(calculateDateForDay(projectStartDate, day)).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2"><div className="w-4 h-4 bg-green-500 rounded-md"></div><span className="text-xs text-slate-600">Completed</span></div>
              <div className="flex items-center space-x-2"><div className="w-4 h-4 bg-blue-500 rounded-md"></div><span className="text-xs text-slate-600">In Progress</span></div>
